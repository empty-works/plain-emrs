...
CREATE TABLE IF NOT EXISTS `plain_emrs`.`diagnosis` (
  `date` DATETIME NULL,
  `diagnosis` BLOB NULL,
  `medical_record_record_id` INT UNSIGNED NOT NULL,
  `medical_record_patient_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`medical_record_record_id`, `medical_record_patient_id`),
  CONSTRAINT `fk_diagnosis_medical_record1`
    FOREIGN KEY (`medical_record_record_id` , `medical_record_patient_id`)
    REFERENCES `plain_emrs`.`medical_record` (`record_id` , `patient_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
