...
CREATE TABLE IF NOT EXISTS `plain_emrs`.`patient_emergency_contacts` (
  `emergency_contact_id` VARCHAR(45) NOT NULL,
  `patients_patient_id` VARCHAR(75) NOT NULL,
  PRIMARY KEY (`emergency_contact_id`, `patients_patient_id`),
  INDEX `fk_table1_emergency_contact1_idx` (`emergency_contact_id` ASC) VISIBLE,
  INDEX `fk_patient_emergency_contacts_patients1_idx` (`patients_patient_id` ASC) VISIBLE,
...
START TRANSACTION;
USE `plain_emrs`;
INSERT INTO `plain_emrs`.`users` (`username`, `password`, `email_address`, `enabled`, `created_on`) VALUES ('the_admin', '{bcrypt}$2a$12$N1McSGCqswsWEcZA7iWVTeA2z5v06LeAs3pWpBJ/bfkMd5xy/M4fO', 'the_admin@gmail.com', true, '2021-08-05 21:57:01');
