...
CREATE TABLE IF NOT EXISTS `plain_emrs`.`diagnoses` (
  ...
  CONSTRAINT `fk_diagnoses_medical_records1`
    FOREIGN KEY (`record_id` , `username`)
    REFERENCES `plain_emrs`.`medical_records` (`record_id` , `username`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
START TRANSACTION;
USE `plain_emrs`;
INSERT INTO `plain_emrs`.`users` (`username`, `given_name`, `middle_name`, `last_name`, `street_address`, `city`, `state`, `country`, `phone_number`, `date_of_birth`, `linked_to_user`, `password`, `email_address`, `enabled`, `created_on`) VALUES (DEFAULT, 'The', 'Cool', 'Admin', '1 Admin Ave.', 'City', 'State', 'Country', '999-999', '1993-01-23', DEFAULT, DEFAULT, NULL, DEFAULT, DEFAULT);
...
START TRANSACTION;
USE `plain_emrs`;
INSERT INTO `plain_emrs`.`authorities` (`username`, `authority`) VALUES (DEFAULT, 'ROLE_ADMIN');
INSERT INTO `plain_emrs`.`authorities` (`username`, `authority`) VALUES (DEFAULT, 'ROLE_AUTHORIZED');
